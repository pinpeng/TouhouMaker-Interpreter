...
function _step_active(_father, _selected) {
	...
	if(keyboard_check_pressed(vk_up)) {
		if(ds_list_size(history_list)) {
			if(history_index == -1) {
				history_index = ds_list_size(history_list) - 1;
				now_text = text;
			} else {
				history_index = clamp(history_index - 1, 0, ds_list_size(history_list) - 1);
			}
			text = history_list[|history_index];
	...
	if(keyboard_check_pressed(vk_down)) {
		if(ds_list_size(history_list)) {
			if(history_index == -1) {
			} else if(history_index == ds_list_size(history_list) - 1) {
				history_index = -1;
				text = now_text;
				text_index = string_length(text);
			} else {
...
function _draw_active(_father, _alpha) {
	...
	draw_set();
	draw_surface_ext(surf, x - left, y - top, 1, 1, 0, -1, _alpha);
	
	draw_set_color($ff0000);
	draw_set_color($3f3f3f);
	draw_rectangle(
	x - left - 1, y - top - 1,
	x + right, y + bottom, true);
	draw_rectangle(
	x - left - 2, y - top - 2,
	x + right + 1, y + bottom + 1, true);
	draw_set();
	bm_reset
